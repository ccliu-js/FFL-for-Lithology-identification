...
class SliceData:
    ...
    def slice_signal_data(self):
        ...
        for class_name, class_list_data in tqdm(
            self.data.items(), desc="Slicing signal data"
        ):
            cache_data = []
            for single_signal in tqdm(
                class_list_data, desc=f"Processing {class_name}", leave=False
            ):
                cache_data.extend(
                    self.slice_single_signal(
                        single_signal,
    ...
    def slice_single_signal(self, signal, slice_len, overlap, drop_last=True):
        """Slice a one-dimensional signal, optionally using overlap."""
        assert 0 <= overlap < 1, "overlap must be in the range [0, 1)"

        step = int(slice_len * (1 - overlap))
        if step <= 0:
            raise ValueError("overlap is too large and produces step <= 0")
